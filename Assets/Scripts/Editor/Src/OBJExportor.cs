...
// 	}
    ...
    public static void ExportOBJ(BuildTarget buildTarget, Object[] objs)
    {
        ...
        foreach (Object obj in objs)
        {
            ...
            Debug.Log("Export Obj Type"+ obj);
            string exportPath = Path.GetDirectoryName(objPath).Replace("Assets",rootPath) + "/";
            ...
            build.assetBundleName = EditorUtils.GetAssetBundleName(objPath, fileName);

            BuildPipeline.BuildAssetBundles(exportPath, new AssetBundleBuild[] { build }, EditorUtils.OPTION, buildTarget);

            EditorUtils.RenameFile(exportPath, build.assetBundleName.ToLower(), fileName);
        }
        Debug.Log("Export over");
    }
}
