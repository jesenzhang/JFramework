...
namespace VFrame.ABSystem
{
    public class AssetBundleBuilder5x : ABBuilder
    {
        ...
        public void ExportInTargetSavePath()
        {
            base.Export();
            ...
            for (int i = 0; i < all.Count; i++)
            {
                ...
                if (target.needSelfExport)
                {
                    ...
                    list.Add(build);
                    BuildPipeline.BuildAssetBundles(target.bundleSavePath, list.ToArray(), option, EditorUserBuildSettings.activeBuildTarget);
                }
            }
        }
        ...
        public override void Export()
        {
            ...
            for (int i = 0; i < all.Count; i++)
            {
                ...
                if (target.needSelfExport)
                {
                    ...
                    build.assetNames = new string[] { target.assetPath };
                    list.Add(build);
                    Debug.Log("标记所有 asset bundle name " + target.assetPath + "   " + " " + target.bundleName + " " + target.bundleCrc);
                }
            }
