...
namespace VFrame.ABSystem
{
    ...
    public class AssetTarget : System.IComparable<AssetTarget>
    {
        ...
        public AssetTarget(Object o, FileInfo file, string assetPath)
        {
            ...
            /* string exportPath = Path.GetDirectoryName(assetPath)
            .Replace("Assets", AssetBundleUtils.pathResolver.BundleSavePath)
            .Replace('\\', '/')
            .Replace(AssetBundlePathResolver.instance.BundleResourceEditor, "")
       ...
       */
        ...
        public void Analyze()
        {
            ...
            var res = from s in deps
                      ...
                      select obj;
            var paths = res.Distinct().ToArray();//	通过使用的默认相等比较器对值进行比较从序列返回非重复元素。
