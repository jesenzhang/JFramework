...
namespace VFrame.ABSystem
{
    public class AssetBundleBuildPanel : EditorWindow
    {
        ...
        static void Open()
        {
            ...
            if (!Directory.Exists(savePathParent))
            {
                Directory.CreateDirectory(savePathParent);
            }
        }
        ...
        const string savePathParent = "Assets/ABSystem";
        ...
        void OnGUI()
        {
            ...
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    EditorGUILayout.PrefixLabel("Compression");
                    var indent = EditorGUI.indentLevel;
                    EditorGUI.indentLevel = 1;
                    AssetBundleBuildConfig.CompressOptions cmp = (AssetBundleBuildConfig.CompressOptions)EditorGUILayout.IntPopup(
                      
                        (int)_config.m_Compression,
                         _config.m_CompressionOptions,
                        _config.m_CompressionValues);

                    if (cmp != _config.m_Compression)
                    {
                        _config.m_Compression = cmp;
                    }
                }
                GUILayout.EndHorizontal();
                GUILayout.Space(10);
